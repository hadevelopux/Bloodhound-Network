...
-->
<template>
  <UiWidgetPanel :title="$t('recurrent_conn_title')">
    ...
    <div class="flex justify-between text-[10px] text-stone-500 font-bold mb-2 uppercase tracking-wider px-1">
      <span>{{ $t('ip_origin_dest_col') }}</span>
      <span>{{ $t('pkts_sent_col') }}</span>
    </div>
    
    <div class="space-y-2">
      <div v-for="conn in connections" :key="conn.id" class="flex justify-between items-center p-2 bg-stone-50 dark:bg-[#1a1a1a] rounded text-xs border border-stone-100 dark:border-stone-800 hover:border-neon-cyan/30 transition-colors">
        <span class="text-stone-600 dark:text-stone-400 font-mono truncate mr-2" v-html="escapeHTML(conn.id)"></span>
        <span class="text-stone-800 dark:text-stone-300 font-bold shrink-0">{{ conn.count }}</span>
      </div>
      <div v-if="!connections || connections.length === 0" class="text-xs text-stone-500 dark:text-stone-500 text-center py-2 italic">
        {{ $t('no_connections') }}
      </div>
    </div>
  </UiWidgetPanel>
